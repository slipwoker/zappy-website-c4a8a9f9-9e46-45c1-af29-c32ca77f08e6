document.addEventListener('DOMContentLoaded', function () {
      ...
      if (value !== '') {
        ...
        if (field.hasAttribute('minlength')) {
          ...
          if (value.length < minLen) {
            showFieldError(field, getErrorMessage(field, 'minlength', minLen));
            isValid = false;
            if (!firstInvalidField) firstInvalidField = field;
            return;
          }
        ...
        if (field.hasAttribute('maxlength')) {
          ...
          if (value.length > maxLen) {
            showFieldError(field, getErrorMessage(field, 'maxlength', maxLen));
            isValid = false;
            if (!firstInvalidField) firstInvalidField = field;
            return;
          }
  ...
  function getErrorMessage(field, type, extra) {
    ...
    const customMessages = {
      required: field.getAttribute('data-error-required'),
      email: field.getAttribute('data-error-email'),
      tel: field.getAttribute('data-error-tel'),
      minlength: field.getAttribute('data-error-minlength'),
      maxlength: field.getAttribute('data-error-maxlength'),
      pattern: field.getAttribute('data-error-pattern')
    };
    ...
    const defaults = {
      ...
      maxlength: label + ' must be no more than ' + extra + ' characters.',
      pattern: label + ' format is invalid.'
    };

    return defaults[type] || 'Invalid value.';
  }
    ...
    "translations": {
      "he": {
        "consentModal": {
          ...
          "description": "מועצת הנוער העירונית אשקלון משתמש בעוגיות כדי לשפר את החוויה שלך, לנתח שימוש באתר ולסייע במאמצי השיווק שלנו.",
    ...
    (function() {
        ...
        function initContactForm() {
            ...
            try {
                ...
                const payload = {
                    websiteId: 'c4a8a9f9-9e46-45c1-af29-c32ca77f08e6',
                    name: name,
                    email: email,
                    subject: subject,
                    message: message,
                ...
                if (result.success) {
                    ...
                    if (result.thankYouPagePath && result.ticketNumber) {
                        ...
                        if (isPreviewMode && window.ZAPPY_CONFIG) {
                            ...
                            const websiteId = window.ZAPPY_CONFIG.websiteId || 'c4a8a9f9-9e46-45c1-af29-c32ca77f08e6';
            ...
            if (mobileToggle && navMenu) {
                ...
                if (phoneHeaderBtn) {
                    phoneHeaderBtn.addEventListener('click', function() {
                        ...
                        // Falls back to +972587157050 placeholder which businessInfoUpdater can replace
...
(function(){
  try {
    ...
    document.addEventListener('submit', function(e) {
      ...
      var wid = 'c4a8a9f9-9e46-45c1-af29-c32ca77f08e6';
